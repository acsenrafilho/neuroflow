/**
 * 3D Slicer tool page: dynamic form per module, batch uploads, job submit, log polling.
 */
(function (global) {
  const modules = global.SLICER_MODULES || {};
  ...
  let activeModuleId = queryModuleId || "slicer-dwi-convert";
  let hoursPerScan = 0.1;
  let pollTimer = null;
  let currentJobId = null;
  const TOOL_ID = "slicer";
  ...
  function modulePageUrl(moduleId) {
    return `/tools/slicer.html?module=${encodeURIComponent(moduleId)}`;
  }
  ...
  function renderPrerequisites(config) {
    ...
    prerequisitesList.innerHTML = config.prerequisites
      .map((step, index) => {
        const link =
          step.moduleId != null
            ? `<a href="${modulePageUrl(step.moduleId)}" class="font-semibold text-secondary hover:underline">${modules[step.moduleId]?.moduleName || step.moduleId} module</a>`
            : "";
  ...
  function renderInputs(config) {
    ...
    config.inputs.forEach((inputDef) => {
      ...
      block.innerHTML = `
        ...
        <input
          type="file"
          class="slicer-file-input rounded-lg border border-outline-variant bg-surface-container-lowest px-3 py-2 text-sm"
      ...
      const fileInput = block.querySelector(".slicer-file-input");
  ...
  function updateCliPreview() {
    ...
    const launch = activeConfig.launchName || activeConfig.moduleName;
    const lines = [
    ...
    lines.push(`./Slicer --launch ${launch} … (same command per input, run sequentially on host)`);
  ...
  async function loadModule() {
    try {
      ...
      if (apiModule) {
        ...
        document.getElementById("page-title").textContent = `3D Slicer · ${apiModule.module_name}`;
    ...
    if (!activeConfig) {
      activeModuleId = "slicer-dwi-convert";
      activeConfig = modules["slicer-dwi-convert"];
    }

    if (!document.getElementById("page-title").textContent.includes("3D Slicer")) {
      document.getElementById("page-title").textContent = `3D Slicer · ${activeConfig.moduleName}`;
    ...
    if (docsLink && activeConfig.docsUrl) {
      ...
      docsLink.textContent = activeConfig.docsLabel || "3D Slicer documentation";
    }
  ...
  async function pollJob(jobId) {
    const logData = await NeuroflowApi.fetchJson(`/api/v1/tools/slicer/jobs/${jobId}/log`);
    const statusData = await NeuroflowApi.fetchJson(`/api/v1/tools/slicer/jobs/${jobId}`);
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const { res } = await NeuroflowApi.fetchApi("/api/v1/tools/slicer/jobs", {
        method: "POST",
        body: formData,
      });
