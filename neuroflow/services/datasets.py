...
def normalize_subject_id(subject_id: str) -> str:
    ...
    if not cleaned or not _SUBJECT_RE.match(cleaned):
        raise ValueError("Subject ID must contain only letters, numbers, underscores, and hyphens")
...
class DatasetStore:
    ...
    def ensure_subject_tree(self, workspace: str, subject_id: str, modality: Modality) -> Path:
    ...
    def derivative_dir(
        ...
        path = self.subject_dir(workspace, subject_id) / "derivatives" / package_id / folder
        path.mkdir(parents=True, exist_ok=True)
        return path
