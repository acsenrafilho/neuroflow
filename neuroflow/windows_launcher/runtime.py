...
from neuroflow.windows_launcher.arch import is_arm_arch, probe_ubuntu_arch
...
def _start_argv(linux_home: str, linux_elf: str) -> list[str]:
    """Build the allowlisted ``wsl -d Ubuntu -- env …`` start argv (without wsl.exe)."""
    ...
    return [
        ...
        f"NEUROFLOW_PORTAL_PIDFILE={pidfile}",
        linux_elf,
    ]


def _launch_with_lock(probe: WslProbe, *, wait_on_process: bool) -> int:
    """Happy path after the launcher lock is held."""
    assert probe.wsl_exe
    ...
    ubuntu_arch = probe_ubuntu_arch(wsl_exe)
    if is_arm_arch(ubuntu_arch):
        return _fail("NeuroFlow — unsupported architecture", MSG_ARM_REFUSED, code=0)
    ...
    try:
        proc = popen_wsl(
            wsl_exe,
            _start_argv(paths.linux_home, paths.linux_elf),
...
def launch(probe: WslProbe, *, wait_on_process: bool = True) -> int:
    """Run the Phase 2/5 happy path for a ready Ubuntu probe result.
    ...
    with LauncherLock() as lock:
        if not lock.acquired:
            # Another launcher is starting (or holding wait). Prefer browser if up.
            existing = probe_health()
            if existing.status == HealthStatus.OK:
                _open_portal_browser()
                return 0
            console.print(MSG_ALREADY_STARTING)
            notify_user("NeuroFlow", MSG_ALREADY_STARTING)
            return 0
        return _launch_with_lock(probe, wait_on_process=wait_on_process)
